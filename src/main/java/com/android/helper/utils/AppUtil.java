...
public class AppUtil {
   ...
   *
   * <p>在android 11 上面，如果想要去获取到安装的应用信息，需要去增加一个权限
   *
   * <ol>
   *   这个是查询所有的权限 <uses-permission * android:name="android.permission.QUERY_ALL_PACKAGES" *
   *   tools:ignore="QueryAllPackagesPermission" />
   * </ol>
   *
   * <ol>
   *   这个是查询单个的权限 <queries> <package android:name="com.autonavi.minimap" /> </queries>
   * </ol>
   */
    ...
    try {
      if (BaseApplication.getInstance().getApplication() != null) {
        ApplicationInfo applicationInfo =
            BaseApplication.getInstance().getApplication().getApplicationInfo();
